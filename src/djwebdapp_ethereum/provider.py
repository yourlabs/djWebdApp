...
class EthereumEventProvider(EthereumProvider):
    ...
    def find_fork_point(self):
        ...
        # Start checking from the lower of current head and our index level
        start_level = min(current_head, self.blockchain.index_level)

        # Walk backwards from most recent to find last common ancestor
        indexed_blocks = IndexedBlock.objects.filter(
            blockchain=self.blockchain,
            level__lte=start_level,
    ...
    def reorg(self):
        ...
        2. DELETE events at or after fork point (cascades to domain objects)
        3. Reset transactions to state='deleted'
        4. Delete IndexedBlock records
        5. Reset blockchain.index_level to fork point
        ...
        # DELETE events at or after reorg level
        # This cascades to any domain objects that FK to EthereumEvent
        deleted_events = EthereumEvent.objects.filter(
            contract__blockchain=self.blockchain,
            transaction__level__gte=reorg_level,
        ).delete()
        self.logger.info(f'Deleted {deleted_events} events due to reorg')
