...
class TestFindForkPoint:
    ...
    def test_finds_fork_point_when_chain_shrinks_to_reorg_level(
        self,
        mock_provider,
        indexed_blocks,
    ):
        """
        Fork point found when chain is shorter than indexed level all
        chain blocks match.
        """
    ...
    def test_finds_fork_point_when_chain_shrinks_and_grows(
        self,
        mock_provider,
        indexed_blocks,
    ):
        """
        Fork point found when chain is shorter than indexed level. But the
        fork point is still lower that chain head.
        """
        # Indexed up to 100, but chain shrunk to 96
        # and chain re-minted different blocks 96 and 97
        # Need to find where the fork actually happened, which is 95
        mock_provider.client.eth.get_block_number = MagicMock(return_value=95)
        ...
        def get_block(level):
            if level > 97:
                raise Exception("Block not found")
            if level >= 96:
                # Blocks 96-97 have different hashes (part of reorg)
                return {'hash': MagicMock(to_0x_hex=lambda: f'0x{"b" * 64}')}
            return {'hash': MagicMock(to_0x_hex=lambda: f'0x{"a" * 64}')}

        mock_provider.client.eth.get_block = get_block

        # Fork point is at 95 (last matching block), so reorg_level = 96
        result = mock_provider.find_fork_point()
        assert result == 96
...
class TestReorgHandling:
    ...
    def test_reorg_preserves_events_and_marks_transactions_deleted(
        self, mock_provider, blockchain, indexed_blocks
    ):
        """Events are preserved during reorg, transactions marked as deleted."""
        ...
        # All events are preserved (not deleted)
        assert EthereumEvent.objects.filter(pk=event_before.pk).exists()
        assert EthereumEvent.objects.filter(pk=event_at_reorg.pk).exists()
        assert EthereumEvent.objects.filter(pk=event_after_reorg.pk).exists()

        # Transactions at/after reorg are marked deleted
        tx_at_reorg.refresh_from_db()
        tx_after_reorg.refresh_from_db()
        assert tx_at_reorg.state == 'deleted'
        assert tx_after_reorg.state == 'deleted'

        # Transaction before reorg is unchanged
        tx_before.refresh_from_db()
        assert tx_before.state == 'done'

    def test_reorg_marks_transactions_deleted_preserving_data(
        self, mock_provider, blockchain, indexed_blocks
    ):
        """Transactions at or after reorg are marked deleted but data is preserved."""
        tx_before = EthereumTransaction.objects.create(
            blockchain=blockchain,
            hash='0x' + '1' * 64,
            level=95,
        ...
        # Transaction at reorg: state is deleted but data is preserved for audit
        assert tx_at_reorg.state == 'deleted'
        assert tx_at_reorg.level == 98  # Preserved for audit
        assert tx_at_reorg.hash == '0x' + '2' * 64  # Preserved for audit
        assert tx_at_reorg.address == '0x' + 'a' * 40  # Preserved for audit
        assert tx_at_reorg.normalized is True  # Preserved
    ...
    def test_reorg_emits_signal_with_details(
        self, mock_provider, blockchain, indexed_blocks
    ):
        """reorg_detected signal is emitted with blockchain, level, and block hash."""
        ...
        def signal_handler(sender, **kwargs):
            signal_calls.append({
                ...
                'reorg_level': kwargs.get('reorg_level'),
                'reorg_block_hash': kwargs.get('reorg_block_hash'),
            })

        # Connect handler
        reorg_detected.connect(signal_handler)
        ...
        try:
            ...
            assert call['sender'] == EthereumEventProvider
            assert call['blockchain'] == blockchain
            assert call['reorg_level'] == 98
            # Block hash is from the indexed block at reorg level (the old hash)
            assert call['reorg_block_hash'] == f'0x{"a" * 64}'
        finally:
            # Disconnect handler to avoid affecting other tests
            reorg_detected.disconnect(signal_handler)

    def test_reorg_signal_not_emitted_when_no_reorg(
        self, mock_provider, blockchain, indexed_blocks
    ):
        """reorg_detected signal is NOT emitted when no reorg is detected."""
        from djwebdapp.signals import reorg_detected
        ...
        try:
            # Mock no reorg - all hashes match
            def get_block(level):
                return {'hash': MagicMock(to_0x_hex=lambda: f'0x{"a" * 64}')}
...
class TestCascadeDelete:
    ...
    def test_reorg_hooks_clean_up_domain_objects(
        self, mock_provider, blockchain, indexed_blocks
    ):
        """
        Full reorg flow: reorg() calls reorg hooks which clean up
        domain objects like FA12EthereumBalanceMovement records.
        ...
        # Events are preserved (not deleted) for audit purposes
        assert EthereumEvent.objects.filter(pk=event_at_reorg.pk).exists()
        assert EthereumEvent.objects.filter(pk=event_before.pk).exists()

        # Transaction at reorg level is marked deleted
        tx_at_reorg.refresh_from_db()
        assert tx_at_reorg.state == 'deleted'
    ...
    def test_reorg_hook_called_for_affected_events(
        self, mock_provider, blockchain, indexed_blocks
    ):
        """
        Normalizer's reorg_{EventName} hook is called for affected events.
        """
        from djwebdapp.models import Account
